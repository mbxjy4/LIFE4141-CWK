...
vcf1 <- read.vcfR("LAB_NEN_ODN.clean_BI.ann.3mbChr5.vcf") #Filtered and LD-pruned VCF file
...
#Adding location column to colour samples by location
pcs$location <- "NA"
pcs$location <- c("LAB", "LAB", "LAB", "LAB", "LAB", "LAB", "LAB", "LAB",
                  "NEN", "NEN", "NEN", "NEN", "NEN", "NEN", "NEN",
                  "ODN", "ODN", "ODN", "ODN", "ODN", "ODN", "ODN")

ggplot(pcs, aes(PC1, PC2, color=as.factor(location)))+
  geom_point(size=3)+
  labs(x=xlab, y=ylab, color="location")+
  geom_text_repel(aes(label=id), size=2, force=10, color="black")+
